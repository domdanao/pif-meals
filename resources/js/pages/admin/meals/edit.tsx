...
export default function EditMeal({ meal, categories }: Props) {
    const { data, setData, patch, processing, errors } = useForm({
        ...
        is_halal: meal.is_halal || false,
        contains_nuts: meal.contains_nuts || false,
        contains_dairy: meal.contains_dairy || false,
        allergen_info: meal.allergen_info || '',
        image_url: meal.image_url || '',
        quantity_available: meal.quantity_available || 0,
        is_active: meal.is_active ?? true,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(`/admin/meals/${meal.id}`);
    };

    return (
        <AppLayout>
            <Head title={`Edit ${meal.name} - PIF Meals Admin`} />
                        ...
                        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 rounded-b-xl">
                            <div className="flex items-center justify-between">
                                ...
                                >
                                    ...
                                    {processing ? 'Updating...' : 'Update Meal'}
                                </button>
                            </div>
                        </div>
                    </form>
